...
document.getElementById('totalEntries').addEventListener('click', () => {
    const totalEntriesElement = document.getElementById('totalEntries');
    
    // Check if copy.ico is currently displayed
    const isCopyIconActive = totalEntriesElement.querySelector('img[src="copy.ico"]') !== null;

    if (!isCopyIconActive) {
        // Select a random word from entryWords
        const entries = Object.keys(entryWords);
        if (entries.length > 0) {
            const randomEntry = entries[Math.floor(Math.random() * entries.length)];

            // Update the URL hash with the selected word
...
function updateTotalEntriesDisplay() {
    const searchBox = document.getElementById('searchBox');
    const totalEntriesElement = document.getElementById('totalEntries');
    const query = searchBox.value.trim(); // Get the current search query

    if (query === "") {
        // If search box is empty, show total entry count
        const wordCount = Object.keys(entryWords).length;
        totalEntriesElement.textContent = `${wordCount}`; // Display the count as text
    } else if (dictionaryData[query]) {
        // If the query matches an entry, show the copy icon
        totalEntriesElement.innerHTML = '<img src="copy.ico" alt="Copy Icon">';
    } else {
        // If query doesn't match any entry, show the word count again
        const wordCount = Object.keys(entryWords).length;
        totalEntriesElement.textContent = `${wordCount}`;
    }
}
